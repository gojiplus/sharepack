...
INCOMPATIBLE_DEPS = ("psycopg", "mysqlclient", "lxml", "cryptography",
                     "uwsgi", "gunicorn-native", "grpcio")
...
class DjangoAdapter:
    name = "django"
    pyodide_packages = ["micropip", "sqlite3", "tzdata"]

    def __init__(self, settings_module, db_files, warnings):
        self.settings_module = settings_module
        self.db_files = db_files
        self.warnings = warnings

    @classmethod
    def detect(cls, project: Path, files: dict):
        if "manage.py" not in files:
            return None
        m = re.search(r"""['"]([\w.]+\.settings[\w.]*)['"]""",
        ...
        if re.search(r"postgresql|mysql|oracle", settings_src):
            warnings.append("non-SQLite database backend in settings; "
                            "sharepack only ships SQLite")
        reqs = files.get("requirements.txt", b"").decode("utf-8", "replace").lower()
        for bad in INCOMPATIBLE_DEPS:
            if bad in reqs:
                warnings.append(f"requirements.txt mentions '{bad}'; "
                                "compiled deps may not load in WASM")
        db = [f for f in files if Path(f).suffix.lower() in DATA_EXT]
        if not db:
            warnings.append("no SQLite file found; app must work with an "
                            "empty/no database")
