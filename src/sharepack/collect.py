...
SKIP_DIRS = {"__pycache__", ".git", ".hg", "venv", ".venv", "env", "node_modules",
             ".mypy_cache", ".pytest_cache", ".ruff_cache", ".idea", ".vscode",
             ".tox", "htmlcov", "dist", "build", ".eggs"}
SKIP_FILES = {".env", ".env.local", ".env.production", ".env.development"}
SKIP_SUFFIXES = {".pyc", ".pyo", ".log"}
TEXT_EXT = {".py", ".html", ".txt", ".css", ".js", ".json", ".md", ".cfg",
            ".ini", ".toml", ".yaml", ".yml"}
DATA_EXT = {".sqlite3", ".db", ".sqlite"}

SECRET_KEY_RE = re.compile(r"""^(\s*SECRET_KEY\s*=\s*)(['"]).*?\2""", re.M)
CRED_NAME_RE = re.compile(r"(secret|token|password|credential|apikey|api_key)",
                          re.I)
...
def collect(project: Path):
    ...
    for p in sorted(project.rglob("*")):
        ...
        if p.name in SKIP_FILES or (CRED_NAME_RE.search(p.name)
                                    and p.suffix not in {".py", ".html"}):
